<template>
  <div v-if="this.visible" class="absolute w-full px-4 h-auto z-10">
    <client-only>
      <link
        rel="stylesheet"
        type="text/css"
      ...
      />
      <div
        class="rounded-t-md flex flex-row-reverse px-4 py-2 h-12 bg-white border border-b-0 border-gray-500"
      >
        <button
...
export default {
  ...
  methods: {
    ...
    sendDelta: function() {
      ...
      let a = '↵'
      let delta = this.quill.root.innerHTML
      this.cancelQuillEditor()
...
<style scoped>
@import url('@/static/css/quill.edit.css');
.ql-editor {
  font-family: 'Lato';
}
